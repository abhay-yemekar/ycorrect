/**
 * Local grammar rules — pattern-based checks that run alongside LanguageTool.
 *
 * These catch common issues that the remote API may miss or that we want
 * to enforce regardless of connectivity.
 *
 * Every rule carries a STABLE id: per-document ignore lists are keyed on
 * `rule.id + message`, so an ignore never suppresses unrelated rules that
 * merely share wording, and survives reloads (defect 9).
 */

/**
 * @typedef {object} LocalMatch
 * @property {number} offset
 * @property {number} length
 * @property {string} message
 * @property {{ value: string }[]} replacements
 * @property {{ id: string, issueType: string, category: { name: string } }} rule
 */

const rules = [
  {
    id: 'doubled-intensifier',
    pattern: /\b(very very|really really)\b/gi,
    message: 'This phrase can be more concise.',
    replacement: (m) => m[0].toLowerCase().startsWith('very') ? 'very' : 'really',
    category: 'Clarity',
  },
  {
    id: 'in-order-to',
    pattern: /\bin order to\b/gi,
    message: '"In order to" can usually be simplified to "to".',
    replacement: () => 'to',
    category: 'Clarity',
  },
  {
    id: 'extra-spaces',
    // Horizontal whitespace only — \s would match newlines and report every
    // blank line between paragraphs as an error (v0.2 defect 1)
    pattern: /[ \t]{2,}/g,
    message: 'Remove extra spaces.',
    replacement: () => ' ',
    category: 'Typography',
  },
  {
    id: 'repeated-word',
    pattern: /\b([A-Za-z]+)\s+\1\b/gi,
    message: 'Avoid repeating the same word.',
    replacement: (m) => m[1],
    category: 'Clarity',
  },
  {
    id: 'could-of',
    pattern: /\b(could of|should of|would of|might of|must of)\b/gi,
    message: 'Use "could have" instead of "could of".',
    replacement: (m) => m[0].replace(' of', ' have'),
    category: 'Grammar',
  },
  {
    id: 'your-youre',
    pattern: /\b(your)\s+(a|an|the|very|really|going|own|self)\b/gi,
    message: 'Did you mean "you\'re" (you are)?',
    replacement: (m) => `you're ${m[2]}`,
    category: 'Grammar',
  },
  {
    id: 'its-it-is',
    pattern: /\b(its)\s+(own|way|place|self|time)\b/gi,
    message: 'Check if "its" (possessive) or "it\'s" (it is) is correct here.',
    replacement: null, // flag only, no auto-replace
    category: 'Clarity',
  },
  {
    id: 'common-typo',
    pattern: /\b(teh|adn|hte|taht|wiht|thn|fro|fo)\b/gi,
    message: 'Possible typo.',
    replacement: (m) => {
      const fixes = { teh: 'the', adn: 'and', hte: 'the', taht: 'that', wiht: 'with', thn: 'then', fro: 'for', fo: 'of' };
      return fixes[m[0].toLowerCase()] || m[0];
    },
    category: 'Misspelling',
  },
  {
    // Comprehensive misspelling dictionary — words that LanguageTool
    // often gets wrong (closest edit distance ≠ intended word)
    id: 'misspelling-dict',
    pattern: new RegExp(
      '\\b(' + Object.keys({
        asinged: 'assigned', acheive: 'achieve', acheived: 'achieved',
        occuring: 'occurring', occurence: 'occurrence', ocurrence: 'occurrence',
        definately: 'definitely', definitly: 'definitely', definetly: 'definitely',
        seperate: 'separate', separated: 'separated',
        recieve: 'receive', recieved: 'received', recieving: 'receiving',
        wierd: 'weird', thier: 'their', thr: 'their',
        lenght: 'length', heigth: 'height', widht: 'width',
        neccessary: 'necessary', necesary: 'necessary', neccesary: 'necessary',
        accomodate: 'accommodate', accomodation: 'accommodation',
        publically: 'publically', publicaly: 'publicly',
        independant: 'independent', independance: 'independence',
        existance: 'existence', existense: 'existence',
        persistant: 'persistent', consistant: 'consistent',
        maintainance: 'maintenance', maintainence: 'maintenance',
        adress: 'address', adressed: 'addressed',
        begining: 'beginning',
        comming: 'coming',
        untill: 'until',
        excercise: 'exercise', exersize: 'exercise',
        priviledge: 'privilege', privelage: 'privilege',
        restaraunt: 'restaurant', restruant: 'restaurant',
        amature: 'amateur',
        basicly: 'basically', basiclly: 'basically',
        catagory: 'category', catagories: 'categories',
        collegue: 'colleague',
        commitee: 'committee', committe: 'committee',
        concensus: 'consensus',
        disapear: 'disappear', dissapear: 'disappear',
        embarass: 'embarrass', embarassed: 'embarrassed',
        enviroment: 'environment', enviorment: 'environment',
        feburary: 'february', febuary: 'february',
        foriegn: 'foreign',
        guage: 'gauge',
        happend: 'happened',
        harrassment: 'harassment',
        innosculate: 'inoculate',
        intresting: 'interesting',
        irregardless: 'regardless',
        libary: 'library', liberry: 'library',
        manuever: 'maneuver', maneuver: 'maneuver',
        mischevous: 'mischievous',
        mispell: 'misspell', misspel: 'misspell',
        occassion: 'occasion', ocassion: 'occasion',
        parrallel: 'parallel', paralell: 'parallel',
        passsed: 'passed',
        peice: 'piece', peices: 'pieces',
        posession: 'possession',
        preceed: 'precede', preceeding: 'preceding',
        recomend: 'recommend', reccommend: 'recommend',
        refered: 'referred', refering: 'referring',
        relevent: 'relevant',
        religous: 'religious',
        rythm: 'rhythm', rythym: 'rhythm',
        sacrilige: 'sacrilege',
        sieze: 'seize',
        similiar: 'similar',
        speach: 'speech',
        statment: 'statement',
        sucess: 'success', sucessful: 'successful',
        supercede: 'supersede',
        supposably: 'supposedly',
        temprary: 'temporary',
        threshhold: 'threshold',
        tomorow: 'tomorrow',
        tongiht: 'tonight',
        tragically: 'tragically',
        truely: 'truly',
        tyhat: 'that',
        unforseen: 'unforeseen',
        unfortunatly: 'unfortunately',
        viarious: 'various',
        wether: 'whether',
        wheter: 'whether',
        wich: 'which',
        yaer: 'year',
        yeild: 'yield',
      }).join('|') + ')\\b', 'gi'
    ),
    message: 'Possible misspelling.',
    replacement: (m) => {
      const dict = {
        asinged: 'assigned', acheive: 'achieve', acheived: 'achieved',
        occuring: 'occurring', occurence: 'occurrence', ocurrence: 'occurrence',
        definately: 'definitely', definitly: 'definitely', definetly: 'definitely',
        seperate: 'separate',
        recieve: 'receive', recieved: 'received', recieving: 'receiving',
        wierd: 'weird', thier: 'their', thr: 'their',
        lenght: 'length', heigth: 'height', widht: 'width',
        neccessary: 'necessary', necesary: 'necessary', neccesary: 'necessary',
        accomodate: 'accommodate', accomodation: 'accommodation',
        publically: 'publicly', publicaly: 'publicly',
        independant: 'independent', independance: 'independence',
        existance: 'existence', existense: 'existence',
        persistant: 'persistent', consistant: 'consistent',
        maintainance: 'maintenance', maintainence: 'maintenance',
        adress: 'address', adressed: 'addressed',
        begining: 'beginning',
        comming: 'coming',
        untill: 'until',
        excercise: 'exercise', exersize: 'exercise',
        priviledge: 'privilege', privelage: 'privilege',
        restaraunt: 'restaurant', restruant: 'restaurant',
        amature: 'amateur',
        basicly: 'basically', basiclly: 'basically',
        catagory: 'category', catagories: 'categories',
        collegue: 'colleague',
        commitee: 'committee', committe: 'committee',
        concensus: 'consensus',
        disapear: 'disappear', dissapear: 'disappear',
        embarass: 'embarrass', embarassed: 'embarrassed',
        enviroment: 'environment', enviorment: 'environment',
        feburary: 'february', febuary: 'february',
        foriegn: 'foreign',
        guage: 'gauge',
        happend: 'happened',
        harrassment: 'harassment',
        innosculate: 'inoculate',
        intresting: 'interesting',
        irregardless: 'regardless',
        libary: 'library', liberry: 'library',
        manuever: 'maneuver', maneuver: 'maneuver',
        mischevous: 'mischievous',
        mispell: 'misspell', misspel: 'misspell',
        occassion: 'occasion', ocassion: 'occasion',
        parrallel: 'parallel', paralell: 'parallel',
        passsed: 'passed',
        peice: 'piece', peices: 'pieces',
        posession: 'possession',
        preceed: 'precede', preceeding: 'preceding',
        recomend: 'recommend', reccommend: 'recommend',
        refered: 'referred', refering: 'referring',
        relevent: 'relevant',
        religous: 'religious',
        rythm: 'rhythm', rythym: 'rhythm',
        sacrilige: 'sacrilege',
        sieze: 'seize',
        similiar: 'similar',
        speach: 'speech',
        statment: 'statement',
        sucess: 'success', sucessful: 'successful',
        supercede: 'supersede',
        supposably: 'supposedly',
        temprary: 'temporary',
        threshhold: 'threshold',
        tomorow: 'tomorrow',
        tongiht: 'tonight',
        truely: 'truly',
        tyhat: 'that',
        unforseen: 'unforeseen',
        unfortunatly: 'unfortunately',
        viarious: 'various',
        wether: 'whether', wheter: 'whether',
        wich: 'which', yaer: 'year', yeild: 'yield',
      };
      return dict[m[0].toLowerCase()] || m[0];
    },
    category: 'Misspelling',
  },
  {
    id: 'lowercase-i',
    pattern: /\b(i)\b/g,
    message: '"I" should always be capitalized.',
    replacement: () => 'I',
    category: 'Capitalization',
    // Only match standalone lowercase 'i' — require word boundaries handled by \b
  },
  {
    id: 'as-well-as',
    pattern: /\bas well as\b/gi,
    message: 'Consider using "and" for simpler writing.',
    replacement: () => 'and',
    category: 'Clarity',
  },
  {
    id: 'due-to-the-fact',
    pattern: /\bdue to the fact that\b/gi,
    message: '"Due to the fact that" can be simplified to "because".',
    replacement: () => 'because',
    category: 'Clarity',
  },
  {
    id: 'at-this-point-in-time',
    pattern: /\bat this point in time\b/gi,
    message: 'Can be simplified to "now" or "currently".',
    replacement: () => 'currently',
    category: 'Clarity',
  },
  {
    id: 'in-the-event-that',
    pattern: /\bin the event that\b/gi,
    message: 'Can be simplified to "if".',
    replacement: () => 'if',
    category: 'Clarity',
  },
];

/**
 * @param {string} text
 * @returns {LocalMatch[]}
 */
export function checkLocal(text) {
  if (!text) return [];

  const matches = [];

  for (const rule of rules) {
    // Reset lastIndex for global regexes
    rule.pattern.lastIndex = 0;

    let m;
    while ((m = rule.pattern.exec(text)) !== null) {
      const offset = m.index;
      const length = m[0].length;
      const replacement = rule.replacement ? rule.replacement(m) : null;

      matches.push({
        offset,
        length,
        message: rule.message,
        replacements: replacement ? [{ value: replacement }] : [],
        rule: {
          id: rule.id,
          issueType: rule.category === 'Misspelling' ? 'misspelling' :
                     rule.category === 'Grammar' ? 'grammar' : 'style',
          category: { name: rule.category },
        },
      });
    }
  }

  return matches;
}
